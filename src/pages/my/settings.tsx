...
export default function MyPage(props: Props) {
    ...
    return <Layout>
        <>
            ...
            {
                isLoading ? <p>退会処理中...</p> : data ? <p>退会しました</p> : <button onClick={async () => {
                    if(confirm("本当に退会しますか？あなたが作成したストーリーが全て削除されます。")){
                        await mutateAsync();
                        signOut({
                            callbackUrl: "/"
                        });
                    } 
                }} className={components.buttonDanger} disabled={isLoading}>退会</button>
            }
