...
import {
	calcInputIsCloseToExamples,
	calculateDistanceThreshold,
} from "./calcInputIsCloseToExamples";
...
describe("calcInputIsCloseToExamples", () => {
	test.each([
		...
		},
		{
			examples: example2,
			input:
				"山田さんは人を殺し、男用のトイレにそれを隠そうとしたが、用を足しにきた男たちに見つかりそうになり女用のトイレに入った。",
		},
	])("近い文章が入力された場合trueを返す", async ({ examples, input }) => {
		const [inputEmbedding, ...embeddings] = await Promise.all([
			tioc.getCached(input),
			...examples.map(tioc.getCached),
		]);
	...
	test.each([
		...
		},
		{
			examples: example2,
			input:
				"山田さんは心は女性であり、周りに人がいないような場合は女性用のトイレを使う。",
		},
	])(
		"意味の違う文章が入力された場合falseを返す",
		async ({ examples, input }) => {
			const [inputEmbedding, ...embeddings] = await Promise.all([
				tioc.getCached(input),
				...examples.map(tioc.getCached),
			]);
			expect(calcInputIsCloseToExamples(inputEmbedding, embeddings)).toBe(
				false,
			);
		},
	);
