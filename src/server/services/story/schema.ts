...
export const storyInit = z.object({
  ...
  questionExamples: z.array(questionExample).min(3,"3つ以上の質問を入力してください").refine((data) => {
    const answers = data.map((question) => question.answer);
    return answers.includes("True") && answers.includes("False") && answers.includes("Unknown");
